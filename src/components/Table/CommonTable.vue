...
(new) dynamic add props to 'el-table-column' through 'columnsList' item's options property ---- 2020-05-11 00:21:27
update tempate 'title-key-width' to 'key-title-width' ---- 2020-05-30 19:18:22
(new) generate random data ---- 2020-06-08 15:41:15
change selected data into rawData ---- 2020-06-22 17:35:19
(debug) selected item no-exist when random data on work ---- 2020-07-09 19:00:59
...
export default class extends Vue {
  ...
  filterReg = /\$.*$/
  private turnOnTest = true // useful random data auto-generate feature
  ...
  get innerTableData(): any[] {
    ...
    this.innerRawcolumnsList.forEach((item: ITableHeadItem, index: number) => {
      const _k = item.key
  ...
  generateMap() {
    ...
    this.innerTableData.forEach((item, index) => {
      if (this.turnOnTest && this.tableData.length === 0) {
        _map.set(item, item)
      } else {
        _map.set(item, this.tableData[index])
      }
    })
    this.dataMap = _map
  }

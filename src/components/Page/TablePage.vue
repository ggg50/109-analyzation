<template>
  <div>
    <common-search :search-options="searchOptionsList" @clear="clearParams" @search="onSearch">
      <common-action class="commonAction"
                     :is-default-show="false"
                     :path="path"
                     :add-options-list="addOptionsList"
                     :edit-options-list="editOptionsList"
                     :item-list="innerSelectedRows"
      >
    ...
    >
      ...
      <template #custem="{item}">
        <slot name="custem" :item="item" />
      </template>
    </promise-table>
  </div>
</template>
...
2.
!attention: slot from children components maybe need to do a transit, just like 'custem' slot in this component.

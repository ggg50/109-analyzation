    ...
    <el-row :gutter="50">
      <template v-for="(_, key, index) in innerForm">
        <el-col v-if="showAll || index < showNumber" :key="key" :sm="smCol" :xl="xlCol" class="formItemWrapper">
          <el-form-item :key="key" :label="innerDict[key].label" :prop="key" :class="innerDict[key].className || ''" :style="innerDict[key].style || {}">
            <slot :name="key">
              ...
              <el-select v-else-if="innerDict[key].type === 'select'"
                         v-model="innerForm[key]"
                         style="width: 100%;"
                         :placeholder="innerDict[key].placeholder || `请选择${innerDict[key].label}`"
                         :readonly="readonly"
                         @change="handleSelect('select', key, $event)"
              >
                <template v-for="(item, subIndex) in innerDict[key].keyList">
                  <el-option :key="item" :label="innerDict[key].valueList[subIndex]" :value="item" />
                </template>
              </el-select>
...
1. If item's rules is present, item's 'required' property is set to true automatically, witch is depend on el-form's feature;
...
9. update docx ---- 2020-08-01 22:13:20
10. deep clone for innerDict & data caching ---- 2020-12-09 15:06:49
...
beforeButton|slot|-|-|-| insert something before foot buttons
afterButton|slot|-|-|-| insert something after foot buttons
select|event|-|({key: string, val: any}, type: 'select'|'checkbox')|-|-
...
export default class extends Vue {
  ...
  // created/update inner info depend on formOptions prop
  @Watch('formOptions', { deep: true, immediate: true })
  setInnerData() {
    const _options: IFormItem[] = JSON.parse(JSON.stringify(this.formOptions))
    const _cacheFormData = JSON.parse(JSON.stringify(this.innerForm))
    const _form: IObject = {}
    const _dict: IObject = {}
    ...
    for (const key in _form) {
      if ((_form[key] === '' || _form[key] == null)) {
        _form[key] = _cacheFormData[key]
      }
    }
  ...
  handleSelect(type: 'select' | 'checkbox', key: string, val: number) {
    this.$emit(
      'select',
      { key, val },
      type

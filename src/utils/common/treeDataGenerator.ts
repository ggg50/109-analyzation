...
# update
1.update logic, intro property checking  ---- 2020-08-23 18:46:19
*/

type ITreeItem = {[key: string]: any} // 因为考虑到 'id' 和 'parentId' 这两个属性是可变的，所以采用这样的写法
// interface ITreeItem {
//   id: number | string
...
export function createTreeDataFromItemList(rawList: ITreeItem[], findRootItemFn: ICheckFn, keyDict: IKeyDict = {}) {
  ...
  const _map = new Map() // 存 id - item 对，“用空间换时间”的应用，降低算法复杂度
  const treeData: {[key: string]: any}[] = []

  // check item and create map
  list.forEach(item => {
    if (!item[_id]) {
      throw new Error(`${_id} is required for every item`)
    }
    if (!item[_parentId]) {
      throw new Error(`${_parentId} is required for every item`)
    }

    if (findRootItemFn(item)) {
      treeData.push(item)
    }
  ...
  if (treeData.length === 0) {
    throw new Error('not root item has found! check you item or findRootItemFn')
  }

  list.forEach(item => {
    const _parent = _map.get(item[_parentId])
    if (_parent) {
      if (_parent[_children]) {
        _parent[_children].push(item)
      } else {
        _parent[_children] = [item]
      }
    }
  })
...
// test
// console.log(JSON.stringify(createTreeDataFromItemList(list, (item) => item.parentId === '0'), null, 2))
